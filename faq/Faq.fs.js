...
export const river = (() => {
    let elems;
    const xs = ofArray([mkAttr("className", "variant"), (elems = [reactElement("h2", createObjDebug(singleton(["children", ["Rivière de Bonus"]]))), reactElement("p", createObjDebug(singleton(["children", ["Piochez 5 bonus et mettez-les face visible à côté de la pioche. Chaque fois que vous récoltez des bonus, choisissez parmi les 5, et remplacez-les ensuite pour compléter la rivière de Bonus."]])))], mkAttr("children", reactApi.Children.toArray(Array.from(elems))))]);
    return reactElement("div", createObjDebug(xs));
})();

export const riverEn = (() => {
    let elems;
    const xs = ofArray([mkAttr("className", "variant"), (elems = [reactElement("h2", createObjDebug(singleton(["children", ["Bonus River"]]))), reactElement("p", createObjDebug(singleton(["children", ["Draw 5 bonus and place them face up next to the draw pile. Each time you take some bonuses, choose between the 5 available, and replace them to complete the Bonus River once done."]])))], mkAttr("children", reactApi.Children.toArray(Array.from(elems))))]);
    return reactElement("div", createObjDebug(xs));
})();
