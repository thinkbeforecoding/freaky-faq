...
import { append, singleton as singleton_1, map, delay } from "./.fable/fable-library.3.0.0-nagareyama-rc-007/Seq.js";
...
export const cowEn = new Section("Mad Cow mode", ofArray([new Question(singleton("Is there a limited number of plots for the Mad Cow mode?"), singleton("No. It\u0027s not a problem since it\u0027s really hard to reach. But if it happens to you, use plots of other colors to end the game (and send us your score, it should be impressive!).")), new Question(singleton("When the mad cow moves, does she follow the path of the arrow on the card or does she just go on the destination tile with a new orientation?"), ofArray(["The mad cow only takes the destination parcel. On a U-Turn, she takes the parcel behind her and is now in the opposit direction.", reactElement("p", createObj(singleton(["children", ["Starting from the following position:"]]))), (() => {
    ...
    return reactElement("figure", createObj(singleton(["children", reactApi.Children.toArray(Array.from(children_28))])));
})()])), new Question(singleton("In co-op mode, which fence should we take into account to determine bonus backlash mode?"), singleton("Use the total length of all players\u0027 fences.")), new Question(singleton("Can the Mad Cow win with a Bribe?"), singleton("No, the cow is limited to the same constraints as players. Moreover, if it cannot play a Bribe, the card is discarded.")), new Question(singleton("The Mad Cow uses an Helicopter. Can it cut my fence just after?"), singleton("Yes, it\u0027s a cow.")), new Question(singleton("In coop mode, you can close a fence with multiple players. Is it possible to connect to the last two fences of a partner? Do the partner fence disapear?"), ofArray(["Yes, you can close on the last two fences of a partner, but not on their tractor. You have to remove any fence along plots of the main field.", figures(10, ["img/coop-1.jpg", "img/coop-2.jpg", "img/coop-3.jpg"])]))]));
...
export function render(state, dispatch) {
    ...
    const props_1 = ofArray([["onUrlChanged", (arg) => {
        ...
        return (matchValue_2[0].tag === 1) ? ((matchValue_2[1].tag === 4) ? singleton_1(videosEn) : ((matchValue_2[1].tag === 3) ? singleton_1(variantesEn) : ((matchValue_2[1].tag === 2) ? singleton_1(boutiquesEn) : ((matchValue_2[1].tag === 1) ? singleton_1(montageEn) : ((matchValue_2[1].tag === 5) ? singleton_1(makersEn) : append(singleton_1(introEn), delay(() => append(singleton_1(section(fieldEn)), delay(() => append(singleton_1(section(fenceEn)), delay(() => append(singleton_1(section(moveEn)), delay(() => append(singleton_1(section(powerlessEn)), delay(() => append(singleton_1(section(conceptsEn)), delay(() => append(singleton_1(section(rareEn)), delay(() => append(singleton_1(section(bonusEn)), delay(() => append(singleton_1(section(crazyBonusEn)), delay(() => append(singleton_1(section(groupStagesEn)), delay(() => singleton_1(section(cowEn))))))))))))))))))))))))))) : ((matchValue_2[1].tag === 4) ? singleton_1(videos) : ((matchValue_2[1].tag === 3) ? singleton_1(variantes) : ((matchValue_2[1].tag === 2) ? singleton_1(boutiques) : ((matchValue_2[1].tag === 1) ? singleton_1(montageFr) : ((matchValue_2[1].tag === 5) ? singleton_1(makersFr) : append(singleton_1(intro), delay(() => append(singleton_1(section(field)), delay(() => append(singleton_1(section(fence)), delay(() => append(singleton_1(section(move)), delay(() => append(singleton_1(section(powerless)), delay(() => append(singleton_1(section(concepts)), delay(() => append(singleton_1(section(rare)), delay(() => append(singleton_1(section(bonus)), delay(() => append(singleton_1(section(crazyBonus)), delay(() => append(singleton_1(section(groupStagesFr)), delay(() => singleton_1(section(cow)))))))))))))))))))))))))));
